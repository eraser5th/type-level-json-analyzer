...
export type ParseArray<Tokens extends Token[]> = Tokens extends [
...
> = Tokens extends [
  ...
  ...infer AfterArrayTokens extends Token[],
]
  ? // 閉じカッコが来たら終了
    [AccElements, AfterArrayTokens]
  : ParseResult extends [
        infer Element extends JsonValue,
        infer RemainTokens extends Token[],
      ]
    ? RemainTokens extends [
        SimpleToken.Comma,
        ...infer NextTokens extends Token[],
      ]
      ? // 要素の後にカンマが来たら次の要素へ
...
(function test() {
  ...
  }
  {
    ...
    type expected = [
      [
        UnexpectedToken<SimpleToken.Bad>,
        [SimpleToken.Bad, SimpleToken.RightSquareBracket, SimpleToken.End],
      ],
    ];
