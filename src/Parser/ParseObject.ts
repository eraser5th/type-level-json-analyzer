...
type ParseFieldOnce<Tokens extends Token[]> = Tokens extends [
  StringToken<infer Key>,
  SimpleToken.Colon,
  ...infer Tail extends Token[],
]
  ? ParsePrimitiveValue<Tail> extends [
      infer Value extends JsonValue,
      infer RemainTokens extends Token[],
    ]
    ? [{ [k in Key]: Value }, RemainTokens]
    : [UnexpectedToken<Tail[0]>, Tail]
  : [UnexpectedToken<Tokens[0]>, Tokens];

type ParseFields<Tokens extends Token[], Acc extends object = {}> =
  ParseFieldOnce<Tokens> extends [
    infer Field extends JsonObject,
    infer RemainTokens extends Token[],
  ]
    ? RemainTokens extends [
        SimpleToken.Comma,
        ...infer RemainTokens extends Token[],
      ]
...
export type ParseObject<Tokens extends Token[]> = Tokens extends [
...
(function () {
  {
    type input = [StringToken<"hoge">, SimpleToken.Colon, NumberToken<123>];
